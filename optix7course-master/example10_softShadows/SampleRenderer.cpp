...
namespace osc
{
    ...
    void SampleRenderer::createHitgroupPrograms()
    {
        ...
        hitgroupPGs.resize(5); // 5 programs
        ...
        //metal
        pgDesc.hitgroup.entryFunctionNameCH = "__closesthit__metal";
        pgDesc.hitgroup.entryFunctionNameAH = "__anyhit__radiance";
        ...
        OPTIX_CHECK(optixProgramGroupCreate(optixContext,
                                            ...
                                            &pgOptions,
                                            log, &sizeof_log,
                                            &hitgroupPGs[2]               ///originally is separated only by rays now we also added other materials by amagood
        ));
        if (sizeof_log > 1) PRINT(log);

        //glass
        pgDesc.hitgroup.entryFunctionNameCH = "__closesthit__glass";
        pgDesc.hitgroup.entryFunctionNameAH = "__anyhit__radiance";
        ...
        OPTIX_CHECK(optixProgramGroupCreate(optixContext,
                                            ...
                                            &pgOptions,
                                            log, &sizeof_log,
                                            &hitgroupPGs[3]               ///originally is separated only by rays now we also added other materials by amagood
        ));
        if (sizeof_log > 1) PRINT(log);
        ...
        OPTIX_CHECK(optixProgramGroupCreate(optixContext,
                                            ...
                                            &pgOptions,
                                            log, &sizeof_log,
                                            &hitgroupPGs[4]               ///originally is separated only by rays now we also added other materials by amagood
        ));
        if (sizeof_log > 1) PRINT(log);
    }
        ...
        for (int meshID = 0; meshID < numObjects; meshID++)
        {
            for (int rayID = 0; rayID < RAY_TYPE_COUNT; rayID++)
            {
                ...
                if(rayID == 0)
                {
                    if(mesh->metallic)
                    {
                        OPTIX_CHECK(optixSbtRecordPackHeader(hitgroupPGs[2], &rec));
                        std::cout << "metal" << std::endl;
                    }
                    else if(mesh->transparency)
                    {
                        OPTIX_CHECK(optixSbtRecordPackHeader(hitgroupPGs[3], &rec));
                        //std::cout << "glass" << std::endl;
                    }
                    else if(mesh->emission)
                    {
                        OPTIX_CHECK(optixSbtRecordPackHeader(hitgroupPGs[4], &rec));
                    }
                    else //diffuse
                    {
                ...
                rec.data.emissionColor = mesh->emissionColor;
                rec.data.refractionIndex = mesh->refractionIndex;
    ...
    void SampleRenderer::setCamera(const Camera &camera)
    {
        ...
        const float cosFovy = 0.66f; //50degree
        //const float cosFovy = 0.99980398378478661088073155685039f;  //65degree
    ...
    void SampleRenderer::downloadPixelsAccumulate(float h_pixels[])
    {
        accumulateBuffer.download(h_pixels,
                             launchParams.frame.size.x * launchParams.frame.size.y * 3);
    }
